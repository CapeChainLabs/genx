...
HTTPS_GIT := github.com/capechain/genx.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=genx \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=genxd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/genxd ./cmd/genxd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/genxd.exe ./cmd/genxd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/genxd
...
draw-deps:
	...
	@goviz -i ./cmd/genxd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c genx --local -f chains.yaml
endif
...
set-testnet-configs:
	...
	genxd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | genxd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | genxd keys add acc1 --recover`
