...
# then log output of file /code/bin/genxd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/genxd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/genxd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/genxd /usr/bin/genxd
